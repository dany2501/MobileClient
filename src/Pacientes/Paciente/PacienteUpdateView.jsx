...
function UpdatePacientes({ route ,navigation}) {
    ...
    const getPacientes = async () => {
        ...
        if (data == null) {
            await pacientesController.retrievePacientes(paciente_id, token).then(async (response) => {
    ...
    const buttonHandler = async () => {
        ...
        await pacientesController.updatePacientes(paciente_id, request, token).then(async (response)=>{
            if (response.status === 200){
            ...
            <View className='flex flex-row h-full'>
                <View className='w-full'>
                    <View className='mt-3 ml-5 flex justify-center'>
                        <Text className='text-3xl font-bold'>Actualizar Pacientes</Text>
                    </View>
                            ...
                            <View className='mt-5 w-full items-center flex flex-row'>
                                ...
                                <View className='w-full flex flex-col items-center'>
                                    ...
                                    <View className='w-full flex flex-col justify-between'>
										...
										<Input type={"date"} placeholder={'fecha_nacimiento'} label={'Ingresa la fecha de nacimiento del paciente'} needed={false} onChangeText={setFecha_Nacimiento} value={fecha_nacimiento} />
