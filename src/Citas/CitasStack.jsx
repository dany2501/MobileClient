...
import Cita from './Cita/CitaStack';
import { View, Text, SafeAreaView, StatusBar, TouchableOpacity} from 'react-native';
export default function CitasStackApp() {
    ...
    return (
        <View className={"w-full h-full flex bg-slate-100 px-4 items-center "}>
            ...
            <View className={"w-full items-center justify-center h-full"}>
                <TouchableOpacity className={'w-full h-10 bg-sky-500 rounded-full items-center justify-center'} onPress={()=>{navigation.navigate('Cita')}}>
                    <View>
                        <Text className={'text-white font-bold text-xl'}>Cita</Text>
                    </View>
                </TouchableOpacity>
            </View>
        </View>
  ...
  return (
      ...
      <Stack.Navigator initialRouteName="ChooseScreen">
        ...
        <Stack.Screen name="Cita" options={{ headerShown: false }} component={Cita}/>
      </Stack.Navigator>
  );
